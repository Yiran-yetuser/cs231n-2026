...
def _resolve_device(device):
    """Pick the best available device when none is explicitly requested."""
    if device is None:
...
def train(
    ...
    for data_pair in train_bar:
        ...
        one_left = model(x_i)[1]
        one_right = model(x_j)[1]
        loss = simclr_loss_vectorized(one_left, one_right, temperature, device=device)
...
def train_val(model, data_loader, train_optimizer, epoch, epochs, device=None):
    device = _resolve_device(device)
