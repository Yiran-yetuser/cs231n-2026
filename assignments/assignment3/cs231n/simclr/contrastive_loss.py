...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2 * N):
            if i != k:
                total_sim_k += torch.exp(sim(z_k, out[i]) / tau)
            if i != k + N:
                total_sim_kN += torch.exp(sim(z_k_N, out[i]) / tau)

        Loss_k_kN = -torch.log((torch.exp(sim(z_k, z_k_N) / tau)) / total_sim_k)
        Loss_kN_k = -torch.log((torch.exp(sim(z_k_N, z_k) / tau)) / total_sim_kN)
...
def compute_sim_matrix(out):
    ...
    out_norm = torch.linalg.norm(out, dim=-1, keepdim=True)  # (2N, 1)
    sim_matrix = out @ out.T / (out_norm @ out_norm.T)  # (2N, 2N)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='mps'):
    ...
    numerator = torch.exp(pos_sim / tau)  # [N, 1]

    # Step 4: Now that you have the numerator and denominator for all augmented samples, compute the total loss.
    loss = -torch.log(numerator / denom[:N]) - torch.log(
        numerator / denom[N:]
    )  # [N, 1]
